...
from dask.distributed import Client
from cuml.dask.model_selection import GridSearchCV as DaskGridSearchCV
...
def load_audio_data():
    ...
    df_hard = dh.get_audio_paths("h")
    # join dataframes
    df = cudf.concat([df_easy, df_hard], ignore_index=True)
    print("Loaded audio data")
    return df
...
def extract_audio_features(audio_path):
    ...
    for i, mfcc in enumerate(np.mean(mfccs.T, axis=0)):
        features[f'mfcc_{i}'] = mfcc
    return cudf.Series(features)

def evaluate_audio(df):
    df = df.join(df['audio_path'].applymap(extract_audio_features))
    print("Extracted audio features")
    return df
...
def train(df):
    print("Start training")
    # training and testsplit
    X = df.drop(['label', 'audio_path'], axis=1).to_pandas()
    ...
    grid = DaskGridSearchCV(knn_pipe, param_grid, cv=5, n_jobs=-1)
    grid.fit(X_train, y_train)
